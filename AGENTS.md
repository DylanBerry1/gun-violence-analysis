...
- `src/`: operational scripts for data preparation, map generation, correlation analysis, plotting, and XGBoost modeling.
...
- `data/processed/crime_infrastructure_hex_merged.csv`: generated merged table from the all-crimes correlation stage.
...
- `reports/figures/`: generated plots, correlation figures, summaries, and interactive figures.
...
- That skill is the source of truth for:
  - script order and canonical commands across infrastructure refresh, hex-map generation, correlation analysis, modeling, and rank-order plotting
  - expected raw inputs
...
- Be careful with output-path changes. The combined map HTML lives under `reports/maps/crime_hex_maps/`, the correlation stage writes to `data/processed/crime_infrastructure_hex_merged.csv` plus `reports/figures/`, and downstream modeling still depends on per-crime CSV outputs in `data/processed/hex/`.
