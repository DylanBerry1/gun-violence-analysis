...
def plot_real_crime_rank_order():
    ...
    # log-log plot
    plt.figure()
    plt.loglog(crime_counts, ccdf, marker='o', linestyle='none', markersize=5, alpha=0.7, color='#f16913') # matching your map's orange/red
...
def analyze_and_plot_heavy_tail():
    ...
    df_active = df_active.sort_values(by='count', ascending=False).reset_index(drop=True)
    ...
    fig = px.scatter(df_active, 
                     ...
                     labels={'count': '# of Homicides in Hex (Log)', 
                             'ccdf': 'P(Hex Having > X Homicides) (Log)', 
                             'hex_id': 'Hexagon ID'},
                     color_discrete_sequence=['#f16913']
                     )    
    ## the line where the power law starts (x_min)
    fig.add_vline(x=xmin, line_dash="dash", line_color="gray", annotation_text="  x_min threshold", annotation_position="top right")
